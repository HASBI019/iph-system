...
@section('title', 'Edit IPH Mingguan')

@section('content')
<h2 class="text-2xl font-bold text-indigo-700 mb-6">Edit Data IPH Mingguan</h2>

<form method="POST" action="{{ route('iph-mingguan.update', $data->id) }}" class="space-y-6">
    @csrf
    ...
    <div>
        ...
        <select name="minggu_ke" class="w-full border rounded p-2">
            @for ($i = 1; $i <= 5; $i++)
                <option value="{{ $i }}" {{ $data->minggu_ke == $i ? 'selected' : '' }}>Minggu {{ $i }}</option>
            @endfor
        </select>
    </div>
    ...
    <div class="grid grid-cols-2 gap-4">
        <div>
            ...
            <input type="text" name="perubahan_harga" value="{{ $data->perubahan_harga }}" placeholder="Contoh: 0,1" required class="w-full border rounded p-2">
        </div>
        <div>
